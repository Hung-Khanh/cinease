...
const PaymentDetail = ({ apiUrl, onBack }) => {
  const [ticketData, setTicketData] = useState(null);
  const navigate = useNavigate();
  const { invoiceId, scheduleId } = useParams();
  const [movieName, setMovieName] = useState("");
  const [movieId, setMovieId] = useState(""); // Sửa lỗi khai báo state
    ...
    const fetchTicketDetails = async () => {
      ...
      try {
        const response = await fetch(
          `${apiUrl}/member/ticket-info/${invoiceId}`,
          {
            method: "GET",
            headers: {
              accept: "*/*",
        ...
        setMovieId(data.movieId); // Lưu movieId để fetch poster
        setCinemaRoom(data.cinemaRoomId || data.cinemaRoom || ""); // Lấy cinemaRoom từ ticketData
        setTicketData(data);
      } catch (error) {
  ...
  useEffect(() => {
    // Nếu không có movieId thì không fetch
    if (!movieId) {
      setMovieImage("");
      return;
    }
    const fetchPoster = async () => {
      ...
      try {
        ...
        const data = await response.json();
        // Tìm đúng movie theo movieId
        let movie = Array.isArray(data)
          ? data.find(
              (m) => m.movieId === movieId || m.movieId === parseInt(movieId)
            )
          : null;
        setMovieImage(movie?.largeImage || data[0]?.largeImage || "");
      } catch (error) {
  ...
  const handlePurchase = async () => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/member/ticket-info/${invoiceId}/${scheduleId}`, //
        {
          method: "POST",
          headers: {
            accept: "*/*",
